class Rack::Attack
  ...
  Rack::Attack.cache.store = if ENV["REDIS_URL"].present? && defined?(Redis)
                               ActiveSupport::Cache::RedisCacheStore.new(url: ENV.fetch("REDIS_URL", nil))
                             else
  ...
  throttle("auth/email", limit: 5, period: 5.minutes) do |req|
    req.params["user"]&.dig("email").to_s.downcase.presence if req.path == "/auth/sign_in" && req.post?
  end
  ...
  throttle("analytics/user", limit: 30, period: 1.minute) do |req|
    req.env["HTTP_AUTHORIZATION"].to_s.split.last.presence if req.path == "/api/v1/analytics/route_plan" && req.post?
  end
